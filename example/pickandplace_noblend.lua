...
PTP(p1, SPEED_SEARCH, -1, 0)   -- p_home: 저장된 교시점 이름으로 교체
...
while true do
    ...
    if search_pose ~= nil then
        move_j(search_pose.coordinate, SPEED_SEARCH)
        ...
        if result > 0 then
            if grasp_type == 3 then
                ...
                move_j(push_preapp_p.coordinate, SPEED_APPROACH)      -- pre-approach
                move_l(push_app_p.coordinate,    nil, SPEED_APPROACH) -- approach
                move_l(push_p.coordinate,        nil, SPEED_GRASP)    -- 접촉 지점
                move_l(push_end_p,               nil, SPEED_GRASP)    -- 밀어서 쓰러뜨림
                move_l(push_exit_p,   nil, SPEED_APPROACH) -- retreat
                ...
                move_j(preapp_p.coordinate, SPEED_APPROACH)       -- pre-approach
                move_l(app_p.coordinate,    nil, SPEED_APPROACH)  -- approach
                move_l(grasp_p.coordinate,  nil, SPEED_GRASP)     -- grasp point

                gripper_grip()                                    -- 공압 ON: 화장품 파지/흡착

                move_l(exit_p.coordinate, nil, SPEED_APPROACH)    -- retreat
